		...
		for (unsigned int i = 0; i < bullets->size(); i++)
		{
			...
			for (unsigned int j = 0; j < field->size(); j++)
			{
					...
					bul->GetRadius(), ast->GetRadius()))
				{
					...
					int stage = ast->getStageNumber();

					stage--;

					ast->SetStageNumber(stage);
					...
					if (stage == 2 || stage == 1)
					{
						...
						Asteroid* hello1 = new Asteroid(*ast);
						Asteroid* hello2 = new Asteroid(*ast);


						double angleOfDirection = atan(bul->getdx() / bul->getdy());

							
						int randome = rand() % 15;
						double negativeChange = -1 * randome - 30;
						double positiveChange = randome + 30;


						double negativeAngleChange = angleOfDirection + (negativeChange / 180) * M_PI;
						double positiveAngleChange = angleOfDirection + (positiveChange / 180) * M_PI;

						double hyptoneuses = sqrt(pow(bul->getdx(), 2) + pow(bul->getdy(), 2));

						double negativeX = hyptoneuses* sin(negativeAngleChange);
						double negativeY = hyptoneuses* cos(negativeAngleChange);						
						
						double positiveX = hyptoneuses* sin(positiveAngleChange);
						double positiveY = hyptoneuses* cos(positiveAngleChange);


						hello1->Setdx(negativeX);
						hello1->Setdy(negativeY);

						hello2->Setdx(positiveX);
						hello2->Setdy(positiveY);

						if (stage == 2)
						{
						/*	hello1->SetRadius(0.66 * hello1->GetRadius());
							hello2->SetRadius(0.66 * hello2->GetRadius());*/


							hello1->setMDst(0.66 * hello1->getMdstW(), 0.66 * hello1->getMdstH());
							hello2->setMDst(0.66 * hello2->getMdstW(), 0.66 * hello2->getMdstH());
						}
						else
						{
							/*hello1->SetRadius(0.5 * hello1->GetRadius());
							hello2->SetRadius(0.5 * hello2->GetRadius());*/

							hello1->setMDst(0.5 * hello1->getMdstW(), 0.5 * hello1->getMdstH());
							hello2->setMDst(0.5 * hello2->getMdstW(), 0.5 * hello2->getMdstH());
